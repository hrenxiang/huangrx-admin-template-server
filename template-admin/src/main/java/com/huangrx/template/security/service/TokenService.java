...
import com.huangrx.template.security.AuthenticationUtils;
import com.huangrx.template.security.config.TokenConfig;
import com.huangrx.template.user.token.TokenType;
...
public class TokenService {
    ...
    private static String generateToken(TokenType tokenType) {
        return JWT.create()
                ...
                //签发对象
                .withAudience(AuthenticationUtils.getSystemLoginUser().getUsername())
                //登录类型
                .withClaim(TokenConfig.getTokenTypeKey(), tokenType.getCode())
                // 用户信息缓存的key值
                .withClaim(TokenConfig.getLoginUserKey(), AuthenticationUtils.getSystemLoginUser().getCacheKey())
                //发行时间
    ...
    public SystemLoginUser getLoginUser(HttpServletRequest request) {
        ...
        if (CharSequenceUtil.isNotEmpty(token)) {
            try {
                ...
                // 解析对应的权限以及用户信息
                String uuid = claims.get(TokenConfig.getLoginUserKey()).asString();

                return (SystemLoginUser) cacheCenter.loginUserCache.getObjectOnlyInCacheById(uuid);
